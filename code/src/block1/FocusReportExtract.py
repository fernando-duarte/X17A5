...
def searchURL(cik:str, file_type:str='X-17A-5') -> str:
    ...
    ----------
    cik : str
        The CIK number for a registreed broker-dealer
...
def edgarParse(url:str):
    ...
    # last check to see if response object is "problamatic" e.g. 403 after 100 tries
...
def fileExtract(archive:str) -> list:
    ...
    if pdf_storage.status_code != 200: return None
...
def mergePdfs(files:list) -> PdfFileWriter:
    ...
    for pdf in files:
        ...
        # last check to see if response object is "problamatic" e.g. 403
        if pdf_storage.status_code != 200: continue
